...
function initMap() {
  var locations = [
      ['Jakarta Selatan', -6.270365, 106.775100],
      ['Yogyakarta', -7.755775495273938, 110.33844661534289],
      ['Bandung', -6.9179077326738945, 107.59063789719212],
      ['Surabaya', -7.264711282250993, 112.7551162683585]
    ];
    ...
    if(mapElm) {
      var map = new google.maps.Map(mapElm, {
        zoom: 7,
        center: new google.maps.LatLng(-7.298918383308084, 109.55890299426336),
...
$(function(){
  var $youtubeElm = $('#youtube'),
      $youtubePrincipal = $('#youtube_principal'),
      $cover = $('#banner_cover'),
      $btnPlay = $('#btn_play'),
...
$(function(){
  var $youtubeEvent = $('#youtube_event'),
      $cover = $('#banner_cover'),
      $btnPlay = $('#btn_play'),
      $iconPlay = $('#btn_play .play'),
      $iconPause = $('#btn_play .pause'),
  ...
  $youtubeEvent.YTPlayer({
    ...
    callback: function() {
      videoCallbackEventsMagazine();
    }
  });

  var videoCallbackEventsMagazine = function() {
    var player = $youtubeEvent.data('ytPlayer').player;
    player.addEventListener('onStateChange', function(){
      if(isMobile) {
        player.pauseVideo();
      } else {
